...
class Join(StreamOp):
    ...
    v1 scope: single equality key, 'inner' or 'left' only, right must already be a
    materialized list (not a stream). One-to-many matches are supported (each matching
    right row yields a separate output record); on key collision between a left and
    right record, the right record's fields win.

    Attributes:
        right: The already-materialized list of records to join against.
        on: The jmespath path identifying the join key on left (input) records.
        right_on: The jmespath path identifying the join key on right records. Default:
            None, meaning the same path as on.
    ...
    def __init__(
        ...
        for row in right:
            index.setdefault(dig(row, path=right_on or on), []).append(row)
        self._index = index

    def __call__(self, i: Iterable[Mapping]) -> Iterator[dict]:
        for left_item in i:
            matches = self._index.get(dig(left_item, path=self.on))
            if matches:
